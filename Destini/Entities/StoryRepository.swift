...
/// Протокол хранилища историй
protocol IStoryRepository {

	func getStories() -> [Story]
}

/// Хранилище историй
struct StoryRepository: IStoryRepository {
	...
	private let stories = [
		...
		Story(
			content: "He nods slowly, unfazed by the question.",
		...
		Story(
			content: """
				What? Such a cop out! Did you know traffic accidents are the second leading cause of accidental death \
				for most adult age groups?
				""",
			choices: ["The", "End"], 
			nextStories: [0, 0]
		),
		Story(
			content: """
				...
				the dubious wisdom of stabbing someone while they are driving a car you are in.
				""",
			choices: ["The", "End"],
			nextStories: [0, 0]
		),
		Story(
			content: """
				...
				He drops you off at the next town. Before you go he asks you if you know any good places \
				to dump bodies. You reply: 'Try the pier.'
				""",
			choices: ["The", "End"],
			nextStories: [0, 0]
		)
